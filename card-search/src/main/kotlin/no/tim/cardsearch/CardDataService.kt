...
class CardDataService {
    private val cache = ConcurrentHashMap<String, CardPriceObject>()
    ...
    fun getCardData(): Map<String, CardPriceObject> {
        if (cache.isEmpty()) {
            val s3Client = AmazonS3ClientBuilder.defaultClient()
            ...
            val data: Map<String, CardPriceObject> = objectMapper.readValue(json,
                object: TypeReference<Map<String, CardPriceObject>>() { }
            )
            cache.putAll(data)
        }
        return cache
